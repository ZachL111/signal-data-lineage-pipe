...
`signal-data-lineage-pipe` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies lineage behavior through deny and allow fixtures, with explainable decision traces and local-only command execution.
